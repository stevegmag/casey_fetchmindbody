...
$image_filenames_arr = array(    
  ...
  'Community Night' => 'class_communitynight.jpg',
  'Dance' => 'class_dance.jpg',
  'Fitness' => 'class_fitness.jpg',
  'Happiness/Positivity' => 'class_happinesspositivity.jpg',
  'Healthy Cooking' => 'class_healthycooking.jpg',
  'Healthy Eating' => 'class_healthyeating.jpg',
  'Massage Therapy' => 'class_massagetherapy.jpg',
  'Meditation' => 'class_meditation.jpg',
  'Mindfulness' => 'class_mindfulness.jpg',
  'Nutrition' => 'class_nutrition.jpg',
  'Reiki' => 'class_reiki.jpg',
  'Sleep' => 'class_sleep.jpg',
  'Weight Loss' => 'class_weightloss.jpg',
  'Yoga' => 'class_yoga.jpg'
);
...
// get the feed, using MindBody's supplied php library
...
foreach($result->GetClassesResult->Classes->Class as $one_class){
	$tag_ids_arr[$one_class->ClassDescription->SessionType->Name] = null;// will populate with tag id below
}
...
// for each instructor in the feed, if they have a tribe_organizer post, get the id.  If not, create the post and get the id
...
foreach ($result->GetClassesResult->Classes->Class as $key => $one_class ){
  if (is_string($one_class->Staff->Name)){ 
    if($org_obj = get_page_by_title( $one_class->Staff->Name, OBJECT, 'tribe_organizer' )){
    	$result->GetClassesResult->Classes->Class[$key]->wp_organizer_ID = $org_obj->ID;    //UNFINISHED . . .
    } else {
    	// insert a tribe_organizer post
   	$post_data = array(
        'ID'             => null, // not updating an existing post
        'post_content'   => '', // The full text 
        'post_name'      => null,  // slug.  wp will generate
        'post_title'     => $one_class->Staff->Name,  // The title of your post.
        'post_status'    => 'publish',  // Default 'draft'.
        'post_type'      => 'tribe_organizer', // Default 'post'.
      ...
      //print_r($new_org_id);
      $result->GetClassesResult->Classes->Class[$key]->wp_organizer_ID = $new_org_id;
    }
  }
}
...
function set_post_categories($event_obj, $post_obj){
  ...
  if(isset($event_obj->ClassDescription->SessionType->Name) ){
  	$term_id_arr[] = (int)$tag_ids_arr[$event_obj->ClassDescription->SessionType->Name]; // cast as int because wp_set_object_terms is said to be picky

  // now add the term id's for pillars that this tag belongs to
   if(is_array($pillar_lookup_arr[$event_obj->ClassDescription->SessionType->Name])){
      foreach($pillar_lookup_arr[$event_obj->ClassDescription->SessionType->Name] as $pillar_tag){
